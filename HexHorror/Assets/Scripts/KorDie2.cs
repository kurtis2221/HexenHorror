...
public class KorDie2 : MonoBehaviour {
	...
	IEnumerator SprTimer(float input)
	{
		...
		GetComponent<AudioSource>().PlayOneShot(spirit_snd);
	}
	...
	IEnumerator Credits()
	{
		...
		GameObject.Find("end_cam").GetComponent<Camera>().enabled = true;
		GameObject.Destroy(GameObject.Find("Player"));
		GameObject.Find("credits_text").GetComponent<Renderer>().enabled = true;
